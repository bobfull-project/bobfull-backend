...
AI는 구현과 검증을 수행하고, Human은 정책·계약·최종 승인과 Merge를 책임진다.
...
에이전트는 명령을 받을 때마다 GitHub Issue·PR과 로컬 상태를 다시 읽고, 현재 단계에서 허용된 다음 작업만 수행한다.

에이전트는 Human 작업 후 스스로 다시 실행되지 않는다. Human이 Issue 또는 PR 답변을 작성한 뒤에는 같은 짧은 명령을 다시 입력해 다음 단계부터 재개한다.

## 3. `Issue #번호 구현하라` 상태별 동작

| 현재 상태 | 구현 에이전트 동작 |
|---|---|
| Issue가 `DRAFT`이고 Human 답변이 없음 | Issue·문서·코드를 분석하고 Human 질문을 작성한 뒤 `HUMAN_ANSWER_REQUIRED`로 남기고 중단 |
| Human 답변이 작성됨 | 답변을 검증하고 `AI 보완 설명`을 추가한 뒤 최종 Issue 계약으로 재작성하고 `READY_CANDIDATE`로 남긴 후 중단 |
| Human이 최종 계약을 확인하고 `READY` 승인 | 기능 브랜치 준비 → 구현 → 테스트 → 실제 Diff 검토 → 이해도 질문 → Commit·Push·Draft PR 생성 |
| PR에 Human 답변과 1차 AI 리뷰가 등록됨 | 리뷰를 확인하고 Issue 범위 안의 확정 가능한 지적만 수정·재검증·Push |
| 정책·API·DB 재결정이 필요한 리뷰가 있음 | 임의 수정하지 않고 Human 판단 요청 |
| 수정과 재검증 완료 | `FINAL_HUMAN_REVIEW` 상태로 보고하고 중단 |

대화창의 승인 표현만으로 `READY`를 추정하지 않는다. 실제 GitHub Issue 본문의 상태·체크리스트·승인자를 확인한다.
...
- Human 답변을 AI가 대신 작성한 것처럼 표시하지 않는다.
