...
- 파일 수정 전에 반드시 `git branch --show-current`와 작업 트리 상태를 확인한다.
- 대상 Issue에 연결된 기존 작업 브랜치가 있으면 해당 브랜치로 전환한다.
- 기존 작업 브랜치가 없고 Issue가 구현 가능 상태라면 별도 Human 승인 없이 Issue 전용 브랜치를 생성한다.
- 브랜치명은 저장소의 기존 규칙을 따르며 기본 형식은 `<type>/<issue-number>-<summary>`로 한다.
- 브랜치 생성·전환 후 현재 브랜치를 다시 확인하고, 대상 브랜치가 아니면 구현을 시작하지 않는다.
- 보호 브랜치에서 미커밋 변경을 발견하면 변경을 버리지 않고 추가 수정을 중단한 뒤 Issue 브랜치로 이동한다.
