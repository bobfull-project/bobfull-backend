...
담당자 AI는 명령을 받을 때마다 GitHub Issue, 연결된 PR, 최신 Head, 현재 브랜치와 작업 트리를 다시 읽고 현재 상태의 다음 단계부터 재개한다.
PR 검토·리뷰 반영에는 `PR #번호 검토하라`를 사용한다.
...
2. PR 번호로 연결된 Issue, Issue 댓글의 최종 계약, 현재 상태 Label과 계약 이후 변경 여부를 확인한다.
...
13. 남은 BLOCKER가 없고 필수 Human 입력이 확인되면 `status:final-human-review`를 적용하고 Human 최종 리뷰를 요청한다.
