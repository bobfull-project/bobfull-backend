...
3. 질문별 `일치 | 보완 필요`, 보완 설명, 최종 계약과 구현 진행 또는 중단 판정을 대화창과 별도 Issue 댓글에 기록한다.
4. 답변 누락·모순, 문서 충돌, 정책 결정, 범위 변경, 보안·권한·상태 계약 불명확이 있으면 `status:human-answer-required`를 적용하고 중단한다.
5. 위 문제가 없으면 `status:in-progress`를 적용하고 같은 실행에서 구현을 계속한다.
...
2. `PROJECT_CONTEXT`와 관련 기존 기능·Issue·마일스톤으로 `v1 | v2 | v3 | project` 범위를 판단하고 근거를 작성한다.
3. `ISSUE_TITLE_RULES`로 유형과 제목을 결정한다.
4. `.github/ISSUE_TEMPLATE/feature.md`의 모든 섹션과 순서를 유지하고, 해당 없는 항목은 `해당 없음`과 근거를 기록한다.
5. Human 이해도 질문 수는 작은 변경 1~2개, 일반 기능 3~5개, 권한·결제·좌석·동시성 변경 4~6개 기준을 적용하고 ADR 필요 여부와 근거를 포함한다.

초안 단계에서는 GitHub에 생성·수정을 하지 않는다. Human이 `이 초안으로 Issue 생성하라`고 명시하면 생성 직전 범위와 중복 여부를 다시 확인하고, 승인된 제목·본문만 생성한다. 생성 뒤에는 번호·제목·범위 판단 근거·담당자·마일스톤·Label·URL을 보고한다.
