...
- 기본 검토는 `이해함`을 선택함
- 강화 검토는 `이해함`과 담당자가 직접 작성한 이해 근거 한 줄 이상을 남김

동작:

1. Human 1차 답변을 문서·코드와 대조하고 실제 근거의 `AI 답변 검토`와 `AI 기준 답변`을 작성한다.
2. Human 답변 원문·최종 확인·강화 검토 이해 근거를 덮어쓰거나 대리 작성하지 않는다.
3. 기본 검토에서 `모르겠습니다`여도 AI 기준 답변 뒤 Human이 `이해함`을 선택하면 답변 재작성을 요구하지 않는다. 강화 검토 대상과 직접 작성 이해 근거 조건은 `AI_REVIEW_GUIDE`를 따른다.
...
### 6.1 Issue 단계 Human 이해도

Issue 단계 기존 정책을 유지한다.

- 기본 검토: 필요한 질문 1~2개
- 강화 검토: 필요한 질문 2~3개
- 강화 검토는 `이해함` 외에 담당자가 직접 작성한 이해 근거 한 줄 이상이 필요하다.
- 담당자 AI는 Human 원문·최종 확인·이해 근거를 대신 작성하지 않는다.

### 6.2 PR 단계 Human 이해도

PR 단계는 검토 수준에 따라 질문 수를 고정한다.
...
### 6.3 Human Review Checklist와 댓글
...
- 미검증 범위와 남은 위험
- 기본 검토면 PR Human 이해도 질문 없음, 강화 검토면 정확히 3문항
