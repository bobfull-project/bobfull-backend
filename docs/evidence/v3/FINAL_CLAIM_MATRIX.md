...
- `MEASURED`: 동일 조건 또는 명확한 검증 조건의 Evidence가 있음
- `VERIFIED`: 기능·장애·정합성 검증은 있으나 성능/반복 수치가 핵심이 아님
- `MEASURED_AND_REJECTED`: 측정 후 기술 도입 필요성이 없어 미채택
- `DEFERRED`: 범위상 보류
- `NOT_VERIFIED`: 구현 또는 설계는 있으나 최종 근거 부족

## V3 핵심 Claim Matrix

| Issue | 최종 주장 | 상태 | Primary KPI·현상 | Before | After | Evidence | 핵심 한계 |
|---|---|---|---|---|---|---|---|
