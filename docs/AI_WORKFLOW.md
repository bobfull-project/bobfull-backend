...
기본 검토는 AI 기준 답변 뒤 Human이 `이해함`을 선택하면 답변 재작성 요구 없이 이해 확인을 끝낸다. 강화 검토 대상과 직접 작성 이해 근거 조건은 `AI_REVIEW_GUIDE`를 단일 기준으로 따른다. `추가 설명 필요`는 AI 기준 답변을 보완하고, `동의하지 않음`은 다음 단계로 진행하지 않고 Human 판단을 요청한다.
