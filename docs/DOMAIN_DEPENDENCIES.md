...
→ 합석 테이블·예약 회차
→ 최초 예약 생성 또는 추가 참여
→ 사용자별 partySize 예약금 결제
→ 참여 인원·예약 상태·모집 상태 반영
→ 취소·환불·회차 복구
→ 사장님의 참여자별 노쇼 처리·해제
→ 지급 예정 예약금 조회
```
...
| 결제 | 사용자, partySize, 예약금 | `PAID/FAILED/REFUNDED` | 예약·참여 등록, 환불, 지급 예정금 | 김현승 |
...
| 결제와 참여 등록 순서 변경 | 결제, 예약, 좌석 | 결제 성공 후 저장 실패, 중복 요청, 보상 처리 |
...
1. 결제 실패 시 예약·참여자·현재 참여 인원이 반영되지 않는다.
