...
- 환불 실행은 Reservation 락과 무관하게 참여자별로 수행하고, 완료 시 `RefundCompletionService`가 즉시 응답·PortOne 웹훅 양쪽에서 공통으로 호출하는 완료 경로를 통해 `ReservationCancellationService.completeParticipantCancellation()`을 부른다.
