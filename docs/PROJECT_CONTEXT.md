...
**밥풀**은 사용자가 식당의 합석 회차를 선택해 예약하거나 기존 합석 예약에 참여하는 서비스다. OWNER는 식당·합석 테이블·합석 회차를 관리하고, MEMBER는 `partySize`를 포함해 예약 결제를 준비한다. PortOne 결제 검증 또는 웹훅 검증이 성공하면 예약·참여·결제 결과를 반영한다.
