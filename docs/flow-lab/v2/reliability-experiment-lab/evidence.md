...
| 이메일 후속 처리 경계 | PR #177 Head `0ae7796`의 `NotificationAsyncConfig`, `RecruitmentDeadlineNotificationEventListener`, `ReservationPaymentCompletionNotificationEventListener` | `RecruitmentDeadlineNotificationIntegrationTest`, 두 listener test | Transaction 내부 SMTP·Commit 뒤 동기 SMTP는 비교 모델, Outbox/Kafka는 V3 후속 | `AFTER_COMMIT`은 커밋 경계, `@Async`가 비동기 경계. 서버 종료 시 메모리 Event/Async 유실 가능 |
