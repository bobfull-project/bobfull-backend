...
PR #177은 현재 Head `0ae7796471d236b6e62102eeb6bd662a56d324d8`에서 Event, AFTER_COMMIT listener, 전용 `emailTaskExecutor`, 관련 테스트가 이미 구현되어 있다. 따라서 아래 이메일 항목은 `OPEN PR TARGET · IN PROGRESS`가 아니라 `open PR basis`다. 다음 PR #177 Head 변경 또는 Merge 시 Class·Test·SHA를 다시 동기화한다.
...
| Ch3 CREATE/JOIN 접수·참여 이메일 | open PR basis #177 `0ae7796` | `ReservationConfirmationService.confirm`, `ReservationPaymentCompletionNotificationEventListener.handle`, `NotificationAsyncConfig` | `ReservationConfirmationServiceTest`, `ReservationPaymentCompletionNotificationEventListenerTest` | CREATE 접수는 `RECRUITING`일 수 있어 최종 확정과 구분. Event → AFTER_COMMIT → @Async → SMTP |
...
| Ch6 모집 마감 결과 이메일 | open PR basis #177 `0ae7796` | `ReservationCancellationTransactionService.acceptRecruitmentDeadline`, `RecruitmentDeadlineNotificationEventListener`, `RecruitmentDeadlineCancellationService.process` | `RecruitmentDeadlineNotificationIntegrationTest`, `RecruitmentDeadlineNotificationEventListenerTest`, `RecruitmentDeadlineCancellationServiceTest` | Event는 상태 Transaction 안에서 발행하고 COMMIT 뒤 이메일을 Async 처리. 환불 예외는 Scheduler 경로에 유지되며 이메일 시도와 독립 |
