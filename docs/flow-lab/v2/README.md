...
PR #179 구조화 로그는 기준 `develop`에 병합되어 `develop merged`로 표시한다. PR #177 이메일 알림은 아직 병합되지 않아 현재 실제 Head `0ae7796471d236b6e62102eeb6bd662a56d324d8`의 `open PR basis`로 표시한다. `OPEN PR TARGET · IN PROGRESS`는 Human 확정 목표가 아직 PR Head에 구현되지 않았을 때만 사용하며, 이번 이메일 구조에는 적용하지 않는다.
