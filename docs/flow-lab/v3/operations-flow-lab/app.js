...
function renderCanvas(data) {
  ...
  const nodesSvg = topology.nodes.map(([id, label]) => {
    const [x, y] = topology.nodePositions[id];
  ...
  const badgeSvg = badgeSvgFor(v.badge);
...
function badgeSvgFor(badge) {
  if (!badge) return "";
  const [x, y] = topology.nodePositions[badge.nodeId];
...
function render() {
  ...
  renderCanvas(data); renderComparison(data); renderPerformance(data); renderDetails(data); renderOps(data);
  ...
  $("stepFact").innerHTML = `<span class="badge ${statusClass(data.factStatus)}">${data.factStatus}</span>${quickState(data)}<p>${data.limits || "Evidence 범위 안에서 표시"}</p>`;
