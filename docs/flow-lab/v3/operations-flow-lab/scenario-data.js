  ...
  { id: "redis", shortLabel: "Ch3 — Redis Pub/Sub fan-out",
    ...
    { id: "aws-cross-instance-normal", title: "AWS_CROSS_INSTANCE_NORMAL", steps: [
      ...
      step("publish", "App EC2 #1", "ElastiCache Redis", "↠ publish once", "App EC2 #1이 같은 messageId를 공용 ElastiCache Redis로 발행한다.",
        { domainState: "ChatMessage COMMITTED", redis: "bobfull-ec2-green-1 PUBLISH 확인(messageId=29, 30)", factStatus: FACT.VERIFIED,
  ...
  { id: "kafka-mechanics", shortLabel: "Ch5 — Kafka Mechanics",
    ...
    { id: "async-vs-kafka", title: "ASYNC_VS_KAFKA", steps: [
      ...
      step("send-latency", "Application", "Async Queue / Outbox+Kafka", "◆ send() 응답성 비교", "AI 처리(500ms)를 커밋 후 비동기로 넘기는 건 둘 다 같다 — send() 응답성은 거의 같다.",
        { factStatus: FACT.MEASURED, visual: visual(["app", "db", "async", "outbox", "kafka"], ["commit-async", "outbox-write", "outbox-publish"], "event", null, "outbox", ["db"]),
          ...
          evidenceReferences: [evidence.aiWorkerScaling] }),
      step("drain-compare", "Application", "완료 처리량(drain time)", "◆ 완료 처리량은 오히려 Async가 빨랐다", "Kafka의 Partition key가 chatRoomId라 같은 방 메시지가 한 Partition에 몰려, Consumer 3개 중 1개만 실제로 일했다.",
        { factStatus: FACT.MEASURED, visual: visual(["app", "async", "outbox", "kafka", "consumer"], ["commit-async", "outbox-write", "outbox-publish", "kafka-consume"], "event", null, "outbox", ["db"]),
          ...
          evidenceReferences: [evidence.aiWorkerScaling] }),
      step("reliability", "프로세스 종료", "Async Queue vs Kafka Broker", "× Async 유실 vs ✓ Kafka 보존", "신뢰성 차이가 핵심이다 — Async 대기열은 프로세스가 죽으면 대기 중이던 작업이 재시도 없이 그대로 사라진다.",
        { factStatus: FACT.MEASURED, visual: visual(["async"], ["commit-async"], "failure", "failure", "outbox", ["outbox", "kafka"]),
          logs: "Async Baseline: 큐 대기 중 2건이 강제 종료 시 analyze() 호출 자체가 한 번도 일어나지 않고 사라짐(#192 실험 0, 실측)",
    ...
    { id: "who-retries", title: "WHO_RETRIES", steps: [
      ...
      step("retry-budget", "Human 설정 확인", "Retry 책임 요약", "▲ 실패 위치에 따라 책임이 다르다", "Outbox publish 실패는 Outbox가, Kafka Consumer 처리 실패는 Kafka Consumer가 재시도를 책임진다.",
        { factStatus: FACT.MEASURED, visual: visual(["outbox", "kafka", "consumer", "llm"], [], "commit", null, "kafka"),
