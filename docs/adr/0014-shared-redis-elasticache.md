...
BobFull의 Redis 사용은 Refresh Token 저장소(ADR 0006)에서 시작했고, 이후 Access Token Blacklist, 식당 검색 Cache, 채팅 Redis Pub/Sub으로 범위가 확장됐다.
...
- 검색 Cache namespace/invalidation 상태 불일치
...
- **ADR 0011**은 Redis Pub/Sub을 채팅 실시간 fan-out으로 선택한 메시지 전달 결정이다.
- **ADR 0014**는 다중 App 배포 환경에서 이 Redis 사용처들이 같은 상태를 공유하기 위해 Redis의 **배치·운영 경계 자체를 App EC2 밖의 공용 인프라로 분리한 결정**이다.

따라서 기존 ADR을 대체하지 않고 deployment-level 책임을 보완한다.

## 선택 이유

#169 다중 App 구조에서는 App별 Redis가 분리되면 인증·캐시·채팅 상태가 서로 달라져 서비스 인스턴스 간 일관성을 유지할 수 없다. 공용 Redis는 App을 stateless에 가깝게 유지하고 App 교체/Blue-Green 전환 시에도 같은 공유 상태를 사용할 수 있게 한다.

#169 실제 AWS Evidence에서 서로 다른 App EC2가 공용 ElastiCache Redis를 통해 동일 채팅 메시지를 publish/subscribe하고 양방향 실시간 채팅이 동작함을 확인했다.
...
- 채팅 cross-instance fan-out을 동일 Redis 채널로 처리할 수 있다.

## 단점과 위험

- Redis가 여러 기능의 공용 의존성이므로 장애 영향 범위가 커진다.
- Redis 장애 시 기능별 실패 정책이 서로 다르다. 예를 들어 Access Token Blacklist 조회는 fail-open이지만 Refresh Token 재발급은 fail-closed이고, 채팅 Pub/Sub 누락은 DB cursor 조회로 복구한다.
- ElastiCache 비용과 네트워크/TLS 설정이 추가된다.
- Redis Pub/Sub은 durable queue가 아니므로 공용 ElastiCache로 이전했다고 전달 보장이 생기는 것은 아니다.

## 검증 방법

- 다중 App EC2가 동일 Redis Endpoint를 사용하는지 확인
- Refresh Token/Blacklist/Cache 회귀 검증
...
- Redis HA·replication·cluster topology 요구가 현재 관리형 구성을 넘어설 때
