...
- Redis가 이 프로젝트의 첫 인프라 의존성이 되어, Redis 장애가 로그인 유지(재발급)에 영향을 준다. 이 경우 재발급은 새 토큰을 발급하지 않고 401로 거부된다(fail-closed).
