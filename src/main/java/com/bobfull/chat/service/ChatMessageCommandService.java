...
@Service public class ChatMessageCommandService {
    ...
    private final OutboxEventRepository outboxEvents; private final ChatMessageOutboxProcessor outboxProcessor;
    public ChatMessageCommandService(ChatRoomRepository rooms, ChatMessageRepository messages, ReservationChatAccessReader access, MemberNameReader names, Clock clock,
            OutboxEventRepository outboxEvents, ChatMessageOutboxProcessor outboxProcessor) {
        this.rooms=rooms;this.messages=messages;this.access=access;this.names=names;this.clock=clock;this.outboxEvents=outboxEvents;this.outboxProcessor=outboxProcessor;
    }
    @Transactional public ChatMessageSentResponse send(Long roomId, AuthMember member, String content) {
        ...
        AfterCommitExecutor.run(()->outboxProcessor.signal(outboxEvent.getId()));
