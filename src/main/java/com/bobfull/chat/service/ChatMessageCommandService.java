...
@Service public class ChatMessageCommandService {
    ...
    public ChatMessageCommandService(ChatRoomRepository rooms, ChatMessageRepository messages, ReservationChatAccessReader access, MemberNameReader names) { this.rooms=rooms;this.messages=messages;this.access=access;this.names=names; }
    @Transactional public ChatMessageSentResponse send(Long roomId, AuthMember member, String content) {
        ...
        if(!current.canSend()) throw new CustomException(ChatErrorCode.CHAT_MESSAGE_SEND_NOT_ALLOWED);
