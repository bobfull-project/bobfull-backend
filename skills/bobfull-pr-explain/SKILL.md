...
2. 최신 템플릿의 섹션 이름과 순서를 유지해 한 줄 요약, 관련 Issue, 변경 목적, 변경 전 동작과 문제, 핵심 아이디어와 선택 이유를 작성한다.
...
5. 실제 근거가 있는 예외·실패·중복·경계 상황, 트레이드오프, 현재 제한사항과 확정된 후속 개선만 기록한다.
6. Issue 완료 조건을 구현 위치와 실제 검증 증거에 연결한다. Human 이해도 질문과 PR 전용 Human Review Checklist는 최신 Diff·테스트를 근거로 미체크(`- [ ]`) 항목만 작성한다.
7. PR 본문 마지막의 `Human Review Checklist`와 `담당자 AI 검토·수정 기록`을 유지한다. AI가 Human 답변·리뷰·Approve를 대신 작성하거나 Checklist를 선체크하지 않는다.
