...
- 최초 처리와 재처리 모두 파일 수정 전에 현재 브랜치와 작업 트리를 다시 확인한다. 보호 브랜치(`main`, `master`, `develop`)이거나 미커밋 변경이 있으면 [AGENTS.md](../../AGENTS.md) 브랜치 안전 규칙에 따라 Issue 브랜치로 전환·생성한 뒤에만 수정을 진행한다.
- 모든 문서를 무조건 읽지 않는다.
